...
participant_IDs = unique(data.participant_ID); %everything up until this point is the same as in the import_data script
    ...
    % for the Stroop task
    success_rate_table = table('Size', [length(participant_IDs), 6], ... %the number of rows is the number of participants, and there are 2 columns
                               'VariableTypes', {'double', 'double', 'double', 'double', 'double', 'double'}, ...
                               'VariableNames', {'participant_ID', 'easy_not_easy', 'easy_reaction', 'hard_reaction','reaction_time_stroop', 'success_rate_stroop'});
